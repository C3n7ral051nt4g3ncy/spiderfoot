...
class sfp_circllu(SpiderFootPlugin):
    ...
    def query(self, qry, qtype):
        ...
        if qtype == "PDNS":
            url = f"https://www.circl.lu/pdns/query/{qry}"
        else:
            url = f"https://www.circl.lu/v2pssl/query/{qry}"
        ...
        if res['content'] is None:
            self.info(f"No CIRCL.LU info found for {qry}")
            return None
        ...
        if eventName in ['IP_ADDRESS', 'NETBLOCK_OWNER']:
            ...
            if "/" in eventData:
                ...
                if int(mask) < 23:
                    ...
                    if not ret:
                        self.info(f"No CIRCL.LU passive SSL data found for {eventData}")
            else:
                ret = self.query(eventData, "PSSL")
                if not ret:
                    self.info(f"No CIRCL.LU passive SSL data found for {eventData}")
            ...
            if ret:
                try:
                    ...
                    self.error(f"Invalid response returned from CIRCL.LU: {str(e)}")

        if eventName in ['IP_ADDRESS', 'INTERNET_NAME', 'DOMAIN_NAME']:
            ret = self.query(eventData, "PDNS")
            if not ret:
                self.info(f"No CIRCL.LU passive DNS data found for {eventData}")
                return
            ...
            for line in ret.split("\n"):
                ...
                if (
                    ...
                    and rec['rrtype'] == "A"
                    and rec['rdata'] == eventData
                    and not self.getTarget().matches(rec['rrname'])
                ):
                    # We found a co-host
                    cohosts.append(rec['rrname'])

                if (
                    eventName in ["INTERNET_NAME", "DOMAIN_NAME"]
                    and rec['rdata'] == eventData
                    and not self.getTarget().matches(rec['rrname'])
                ):
                    # We found a co-host
                ...
                for co in cohosts:
                    ...
                    if not self.opts[
                        'cohostsamedomain'
                    ] and self.getTarget().matches(co, includeParents=True):
                        self.debug(f"Skipping {co} because it is on the same domain.")
                        continue
